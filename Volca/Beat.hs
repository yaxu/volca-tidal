...
startbeat = 
  do s <- openUDP "127.0.0.1" 9090
     noteM <- newMVar silence
     ctrlMs <- mapM makeCtrl ["/speed/clap", "/speed/claves", 
                              "/speed/agogo", "/speed/crash", 
                              "/stutter/time", "/stutter/depth", 
                              "/decay/tom", "/decay/clhat", 
                              "/decay/ophat", "/grain/hat"
                             ]
...
ot :: UDP -> MVar (Pattern String) -> [(String, MVar (Pattern Int))] -> Tempo -> Int -> IO ()
...
toNoteMessage :: UDP -> Tempo -> Int -> (Double, String) -> IO ()
toNoteMessage s change ticks (o, sound) 
  = doAt logicalOnset $ sendOSC s $ Message ("/note/" ++ sound) []  
  where beat = fromIntegral ticks / 1
        logicalNow = (logicalTime change beat)
        beat' = (fromIntegral ticks + 1) / fromIntegral 1
